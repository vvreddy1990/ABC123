        ...
        for idx, book_row in unprocessed_books.iterrows():
            ...
            for gidx, gstr2a_row in candidates.iterrows():
                ...
                if (max(legal_name_similarity, trade_name_similarity) >= 70) and (invoice_similarity >= 70):
                    ...
                    if score > best_score:
                        best_score = score
                        best_match = (gidx, gstr2a_row, legal_name_similarity, trade_name_similarity, invoice_similarity)
            if best_match:
                gidx, gstr2a_row, legal_name_similarity, trade_name_similarity, invoice_similarity = best_match
                self.books_df.at[idx, 'processed'] = True
                ...
                book_match_data.update({
                    ...
                    'Tax Sign Status': 'Sign Match',
                    'Value Sign': 'Positive' if book_sign > 0 else 'Negative',
                    'Narrative': self._generate_narrative(book_row, gstr2a_row, False),
                    'Suggestions': self._generate_suggestions(book_row, gstr2a_row, False)
                })
                # Add GSTR-2A record
                gstr2a_match_data = gstr2a_row.to_dict()
                gstr2a_match_data.update({
